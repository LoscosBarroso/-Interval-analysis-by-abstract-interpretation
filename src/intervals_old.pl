...
kleenitialize([b(N,'skip',Next)|CFG],[b(N,'skip',[],[],Next)|CFGV]):-
    kleenitialize(CFG,CFGV).
kleenitialize([b(N,assB(BoolVar,BoolExp),Next)|CFG],[b(N,assB(BoolVar,BoolExp),[],[],Next)|CFGV]):-
    kleenitialize(CFG,CFGV).
kleenitialize([b(N,cond(BoolExp),Next)|CFG],[b(N,cond(BoolExp),[],[],Next)|CFGV]):-
    kleenitialize(CFG,CFGV).
kleenitialize([b(N,assN(numVar(Name),numExp(NumExp)),Next)|CFG],[b(N,assN(numVar(Name),numExp(NumExp)),[vVal(Name,Interval)],[vVal(Name,WidenedInterval)],Next)|CFGV]):-
    nExp2inter(NumExp,[],Interval),
    w('mt',Interval,WidenedInterval),
    kleenitialize(CFG,CFGV).
...
iteration(_,[],[]).
iteration(Ref,[b(N,Block,VarIntList,_,NextBlocks)|CFG],CFGV2):-
    iteration(Ref,CFG,CFGV),
    abstractTransfer(Block,VarIntList,VarIntList1),
    update(CFGV,Ref,[N],VarIntList1,CFGV1),
    update(CFGV1,Ref,NextBlocks,VarIntList1,CFGV2).

update(CFGV,_,[],_,CFGV).
update(CFGV,Ref,[N|Ns],VarIntList,CFGV2):-
    member(b(N,Block,OldVarIntList,VarWideIntList,NextBlocks),CFGV),!,
    delete(CFGV,b(N,Block,OldVarIntList,VarWideIntList,NextBlocks),CFGV1),
